...
class Aggregator(nn.Module):
    ...
    def forward(self, entity_emb, user_emb, edge_index,
        ...
        if self.use_gate:
            ...
            item_emb_fusion = self.cf_module(item_emb_fusion, interact_mat, weight, self.n_items)
            item_emb_fusion = self.attention(item_emb_fusion, user_emb)
...
class GraphConv(nn.Module):
    ...
    def __init__(self, channel, n_hops, n_users, n_relations, n_items, use_gate,
        ...
        for i in range(n_hops):
            self.convs.append(Aggregator(n_users=n_users, n_items=n_items, triplet_attention=self.triplet_attention, use_gate=use_gate, use_cf_module=True, use_attention=True))
